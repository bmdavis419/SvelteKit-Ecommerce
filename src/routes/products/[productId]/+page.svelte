	...
	<!-- DESKTOP -->
		...
		<div class="col-span-3 p-4 relative h-[85vh]">
			{#each data.product.images as im, i}
				<CldImage src={im.cloudinaryId} width={2000} height={1300} objectFit="cover" class={`rounded-lg h-full ${curIdx == i ? 'flex' : 'hidden'}`} />
			{/each}
			...
			<div on:click={() => {curIdx++; if (curIdx == data.product.images.length) curIdx = 0;}} class="absolute cursor-pointer right-10 top-1/2 translate-y-[-50%] bg-gray-300/50 rounded-full flex flex-row justify-center items-center">
				<svg width="48" height="48" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
					<path d="M8.29492 16.59L12.8749 12L8.29492 7.41L9.70492 6L15.7049 12L9.70492 18L8.29492 16.59Z" fill="black"/>
					</svg>
					
			</div>
			<div on:click={() => {curIdx--; if (curIdx == -1) curIdx = data.product.images.length-1;}} class="absolute cursor-pointer left-10 top-1/2 translate-y-[-50%] bg-gray-300/50 rounded-full flex flex-row justify-center items-center">
				<svg width="48" height="48" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
		...
		<div class="p-4 flex flex-col gap-6 h-[85vh] overflow-scroll no-scroll">
			<div class="text-4xl font-jura bg-gradient-to-r from-red-600 via-purple-500 to-indigo-400 text-transparent bg-clip-text">{data.product.name}</div>
			<div class="text-lg text-gray-500 font-light">{data.product.desc}</div>
			<div>
				<span class=" text-3xl font-jura bg-gradient-to-r from-red-600 via-purple-500 to-indigo-400 text-transparent bg-clip-text">Sizes.</span>
				<span class=" text-2xl text-gray-500  font-jura">Which one do you need?</span>
			</div>
			
			{#each data.product.sizes as size, i}
				<div on:click={() => (selectedSizeIdx = i)} class={`${selectedSizeIdx == i ? 'border-blue-500 border-2' : 'border-neutral-600/50 border-[1px]'} w-full cursor-pointer p-6 bg-white rounded-lg flex flex-row justify-between items-center hover:bg-neutral-50  border-solid`}>
...
<style>
	...
	.slides::-webkit-scrollbar-track, .no-scroll::-webkit-scrollbar-track {
		background: transparent;
	}
	/* Hide scrollbar for Chrome, Safari and Opera */
.slides::-webkit-scrollbar, .no-scroll::-webkit-scrollbar {
    display: none;
}
