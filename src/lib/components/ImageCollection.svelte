...
<div class="flex flex-col {collectionData.dark ? 'bg-gray-950 text-white' : 'bg-neutral-100 text-black'} px-2 sm:px-4 pt-6 pb-4 sm:py-10 sm:items-center">
    <div>
    ...
    <a href="/products?tag={collectionData.collectionTag}" class="pb-4 sm:pb-8 flex flex-row items-center">
		...
		<div class="flex flex-row w-full sm:overflow-x-hidden flex-wrap gap-2 sm:justify-center">
			{#each collectionData.productInfo as product}
                <a href={product.link} class="flex flex-col lg:w-[22%] md:w-[30%] sm:w-[40%] w-full sm:p-3 rounded-md cursor-pointer">
